...
from src.roi_selector import load_roi, select_roi
...
def main(video_path, output, config_path, set_roi, roi_config, no_audio, preview):
    ...
    if set_roi:
        click.echo("📐 請在彈出的視窗中框選你的球場範圍...")
        roi = select_roi(video_path, roi_file)
    else:
        try:
            roi = load_roi(roi_file)
        except FileNotFoundError:
            click.echo("⚠️  找不到 ROI 設定檔，請先執行 --set-roi")
            click.echo(f"   python main.py --set-roi {video_path}")
            return

    click.echo(f"📐 ROI: x={roi['x']}, y={roi['y']}, w={roi['w']}, h={roi['h']}")
    ...
    if audio_cfg["enabled"] and not no_audio:
        ...
        click.echo("🔇 音訊分析已停用，使用純視覺模式")

    # === Step 4: Rally 偵測 ===
    rally_cfg = config["rally"]
    click.echo("🏓 偵測 rally 區段...")
    ...
    for i, seg in enumerate(segments):
        click.echo(f"   Rally {i + 1:3d}: {seg.start:7.1f}s - {seg.end:7.1f}s ({seg.duration:.1f}s)")
    click.echo(f"\n   總 rally 時間: {total_rally_time:.1f}s")
